<script>
    ...
    onMount(() => {
      ...
      const createChart = (data, svgElement, color) => {
        ...
        const averageMP = d3.mean(processData, d => d.MP);
        ...
        const y = d3.scaleRadial()
          ...
          .domain([0, d3.max(processData, d => d.MP)]);
        ...
        svg.append("g")
          ...
          .attr("transform", d => `rotate(${(x(d.SEASON) + x.bandwidth() / 2) * 180 / Math.PI - 90})translate(${y(d.MP) + 10},0)`)
          .append("text")
          .text(d => d.SEASON)
      ...
      svg.append("text")
          .text(`Average: ${averageMP.toFixed(2)}`)
  ...
  <div class="chart-container">
    <div class="chart-wrapper">
      <div class="chart">
        <h1>LeBron James Minutes Played</h1>
        <svg bind:this={lbjSvgElement}></svg>
      </div>
      <div class="chart">
        <h1>Michael Jordan Minutes Played</h1>
